...
- 조각에 공간이 충분하면 폴더 이름을 표시합니다. 글자는 원의 접선 방향으로 읽기 쉽게 배치하고, 좁은 조각은 비워 둡니다. 긴 이름은 충분한 글자가 들어갈 때만 줄임표로 표시합니다. 메뉴 막대의 작은 지도에도 적용됩니다.
...
앱 실행 중에는 macOS 상단 메뉴 막대에 작은 원형 게이지와 홈 폴더 용량이 표시됩니다. 위쪽에는 용량 숫자, 아래쪽에는 `GB · HOME`처럼 단위와 범위를 표시합니다. 게이지는 디스크 전체 용량 대비 홈 폴더에서 읽힌 용량의 비율입니다. 누르면 홈 폴더의 작은 용량 지도와 큰 항목 4개가 나타납니다.

- 앱 실행 시 홈 폴더를 스캔합니다. 완료 전에는 `SCANNING`, 완료 후에는 용량을 표시합니다. 메뉴를 닫아도 스캔은 계속되고, 이후에는 마지막 결과를 바로 보여줍니다.
- 지도 조각과 목록을 눌러 폴더 안으로 들어가고, 지도 중심 또는 위쪽 화살표로 돌아옵니다. `홈` 버튼으로 처음 위치로 돌아갈 수 있습니다.
- 새로고침 버튼으로 홈 폴더를 다시 스캔합니다. 업데이트 시각과 접근 제한 항목 수를 함께 표시합니다.
- `큰 창에서 보기`는 현재 결과를 재스캔 없이 앱 창으로 엽니다.
- 앱 창을 닫아도 메뉴 막대 아이콘은 유지됩니다. 완전히 종료하려면 메뉴의 `… → OrbitDisk 종료` 또는 ⌘Q를 사용하세요.

메뉴의 용량은 **홈 폴더에서 읽힌 파일 합계**입니다. 디스크 전체 사용량이나 남은 공간을 뜻하지 않습니다.
